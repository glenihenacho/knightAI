...
| Connector (Tauri) | `apps/connector-tauri` | Runs on customer hardware; pairs, polls, validates RTSP, captures frames, uploads, runs the behavior engine. |
| `surveillance-analysis` | `apps/connector-tauri/src-tauri/analysis` | Pure-Rust Phase 2 engine: YOLOX person detection (tract), tracking, zone/schedule evaluation, rule state machines. No Tauri deps so it unit-tests anywhere. |
